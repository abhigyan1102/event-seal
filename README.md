...
Anyone can verify an event. GitHub sign-in is optional and creates an account on the first successful sign-in. Once signed in, click **Save receipt** on an issued receipt, then open **Dashboard** (`/dashboard`).

Saved references are stored in Postgres, not browser storage. Refreshing the page or signing out does not delete them; sign back into the same account and backend environment to view them. Dashboard results can be filtered by verdict and Solana cluster and are paginated at eight receipts per page. The verifier's current form/result is temporary and resets on refresh. Saving the same receipt again does not create a duplicate.

Only the owning account can read or remove entries from its saved list. Receipt evidence remains publicly readable by receipt ID; saving it does not make the underlying evidence private. Dashboard rows expose the stored verdict, cluster, transaction, trusted program identity, discriminator, and public receipt link.

Before release, verify a real issued receipt can be saved, remains in Dashboard after a hard refresh and sign-out/sign-in, and is absent from another account's Dashboard. Confirm the same behavior against Production before directing customers there; preview data is not transferred by changing the web app's backend URL.
